    ...
    save: function() {
        ...
        while( this._changes.length() ) {
            ...
            if ( record.index ) {
                ...
                ops.push( this._index.del( this.generateIndexKey( record.key, record.oldValue ), 'uniqueID') );
                ops.push( this._index.put( this.generateIndexKey( record.key, record.newValue ), 'uniqueID') );
            }
        }

        // Now put the saveable props as a json block
        ops.push( this._db.put( 'uniqueID', this.generateSaveItem() ) );
