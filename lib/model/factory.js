...
export default class Factory {
    ...
    parse( model ) {
        if ( !model.id ) {
            throw new Error( '[Wrangler] parse expects a valid model with an id' );
        }
    ...
    find( id, opts ) {
        ...
        if ( options.cache ) {
            ...
            // Use private method to grab from the cache, this will throw
            // if there is an error with the id
            try {
                local = this.cache[ this._findCacheIndex({
                    id: id
                })];
            } catch( err ) {
            ...
            if ( local ) {
                this.db.get( local.id, ( err, res ) => {
                    if ( err ) {
                        // Model could exist in cache but never have been saved
                        // to db, so just return.
                        if ( err.notFound ) {
                            return;
                        }
                    ...
                    this.cache.push( this.parse( res ) );
                });
        ...
        return new Promise( ( resolve, reject ) => {
            this.db.createReadStream()
                .on( 'data', res => {
                    ...
                    models.push( this.parse( res.value ) );
                })
                .on( 'error', err => {
                    reject( err );
                })
     ...
     */
    _pushToCache( model ) {
        // Return silently if the model is already in cache
    ...
    _findCacheIndex( model ) {
        if ( !model || !model.id ) {
            throw new Error( '[Wrangler] can not find invalid model' );
        }
