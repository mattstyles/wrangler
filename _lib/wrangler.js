...
module.exports = function( opts ) {
    ...
    var options = _.extend( _.isObject( opts ) ? opts : {}, {
        db: db,
        sep: '\xff'
    });
    // Return new instance and define option defaults
    return new Wrangler( options );
};
...
_.extend( Wrangler.prototype, {
    ...
    addModel: function( model ) {
        ...
        var createModel = function( model ) {
            ...
            model.setDB( this.db.sublevel( model.id, {
                sync: true,
                sublevel: true
            }));
     ...
     *
     * @param id {String}
     * @returns {Model|false} returns false if not found or model not ready
     */
    getModel: function( id, opts ) {
        var model = _.find( this.models, { id: id } );

        if ( !model || !model.ready() ) {
            // @todo - should this throw?
            return opts && opts.sync ? false : Promise.resolve( false );
        }

        return opts && opts.sync ? model : Promise.resolve( model );
    }
