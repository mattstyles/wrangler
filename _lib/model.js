...
_.extend( ModelCtrl.prototype, {
    ...
    find: function( term ) {
        ...
        return new Promise( function( resolve, reject ) {
            ...
            // Find item that matches against multiple indices
            if ( keys.length > 1 ) {
                this._findIndices( term )
                    .then( this.reduceIDs )
    ...
    _findModel: function( uuid ) {
        if ( !uuid ) {
            throw new Error( 'findModel requires an id to find' );
        }
    ...
    /**
     * Takes an array of index:values and returns ids from associated models in
     * each index.
     *
     * @param indices {Array:Object} each item is an index with a value to search for
     * @returns {Promise/Array} array of associated id's
     */
    _findIndices: function( indices ) {
        return Promise.all( _.map( indices, function( val, key ) {
            return this._findIndex( this.generateIndexKey( key, val ) );
        }, this ));
    },
    ...
    /**
     * Reduces a set of id's to a single id. Returns null if the ids do
     * not match.
     *
     * @param ids {Array} array of ids - usually returned by searching indices
     * @return {String || null} returns the id if matching, null otherwise
     */
    reduceIDs: function( ids ) {
        return _.reduce( ids, function( acc, val ) {

            if ( acc === val ) {
                return val;
            }

            // If we got here then the ids are not the same
            return null;
        });
    }
